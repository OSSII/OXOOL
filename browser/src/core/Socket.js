...
app.definitions.Socket = L.Class.extend({
	...
	getWebSocketBaseURI: function(map) {
		return window.makeWsUrlWopiSrc('/oxool/', map.options.doc + '?' + $.param(map.options.docParams));
	},
	...
	connect: function(socket) {
		...
		if (socket && (socket.readyState === 1 || socket.readyState === 0)) {
			...
			try {
				...
				this._map.fire('error', {msg: _('Oops, there is a problem connecting to %productName: ').replace('%productName', (typeof brandProductName !== 'undefined' ? brandProductName : 'OxOffice Online')) + e, cmd: 'socket', kind: 'failed', id: 3});
				return;
			}
		}
	...
	_onSocketOpen: function () {
		...
		// exact same 'oxoolclient' message and a slightly different 'load' message. At least
		// in a "make run" scenario it is that code that sends the 'oxoolclient' and 'load'
		...
		var now2 = Date.now();
		this._doSend('oxoolclient ' + this.ProtocolVersionNumber + ' ' + ((now0 + now2) / 2) + ' ' + now1);
	...
	_onMessage: function (e) {
		...
		if (textMsg.startsWith('oxoolserver ')) {
			...
			if (window.indirectSocket) {
				if (window.expectedServerId && window.expectedServerId != this.WSDServer.Id) {
					if (this.IndirectSocketReconnectCount++ >= 3) {
						var msg = errorMessages.clusterconfiguration.replace('%productName', (typeof brandProductName !== 'undefined' ? brandProductName : 'OxOffice Online Development Edition (unbranded)'));
			...
			if (parseInt(h,16).toString(16) === h.toLowerCase().replace(/^0+/, '')) {
				...
				$('#oxoolwsd-version').html(this.WSDServer.Version + ' <span>git hash:&nbsp;' + h + this.WSDServer.Options + '</span>');
			}
			else {
			...
			this._map.uiManager.showInfoModal('oxool_alert', '', msg, '', _('OK'));
			return;
		}
			...
			if (command.errorKind === 'limitreached' && !this.WasShownLimitDialog) {
				...
				textMsg = textMsg.replace(/{productname}/g, (typeof brandProductName !== 'undefined' ?
					brandProductName : 'OxOffice Online'));
