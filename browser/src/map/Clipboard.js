...
L.Clipboard = L.Class.extend({
	...
	getMetaPath: function(idx) {
		...
		var metaPath = '/oxool/clipboard?WOPISrc=' + encodeURIComponent(this._map.options.doc) +
	...
	_doAsyncDownload: function(type,url,optionalFormData,forClipboard,completeFn,progressFn,onErrorFn) {
		try {
			...
			if ('processOxoolUrl' in window) {
				url = window.processOxoolUrl({ url: url, type: 'clipboard' });
			}
	...
	_isAnyInputFieldSelected: function() {
		...
		if ($('.annotation-active').length && $('.oxool-annotation-edit').is(':visible'))
		    return true;
	...
	_warnFirstLargeCopyPaste: function () {
		...
		var msg = _('If you want to share large elements with other applications (outside of OxOffice Online) it\'s necessary to first download them.');
