	...
	setBounds(bounds: oxool.Bounds) {
		...
		for (let idx = 0; idx < this.cursorWeight; ++idx) {
			...
			boundsForBorder.push(new oxool.Bounds(
				cellBounds.min.subtract(new oxool.Point(pixels, pixels)),
				cellBounds.max.add(new oxool.Point(pixels, pixels))
			));
		}

		const boundsForContrastBorder = new oxool.Bounds(
			cellBounds.min.add(new oxool.Point(1.0, 1.0)),
			cellBounds.max.subtract(new oxool.Point(1.0, 1.0)));
