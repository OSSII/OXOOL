...
class CanvasOverlay extends CanvasSectionObject {
	...
	private ctStart(clipArea?: oxool.Bounds, paneBounds?: oxool.Bounds, fixed?: boolean, freezePane?: { freezeX: boolean, freezeY: boolean }) {
		this.updateCanvasBounds();
		...
		if (this.tsManager._inZoomAnim && !fixed) {
			...
			const clipTopLeft = new oxool.Point(docPos.topLeft.x, docPos.topLeft.y);
	...
	updatePoly(path: CPath, closed: boolean = false, clipArea?: oxool.Bounds, paneBounds?: oxool.Bounds, freezePane?: { freezeX: boolean, freezeY: boolean }) {
		var i: number;
		var j: number;
		var len2: number;
	...
	updateCircle(path: CPath, clipArea?: oxool.Bounds, paneBounds?: oxool.Bounds, freezePane?: { freezeX: boolean, freezeY: boolean }) {
		if (path.empty())
			return;
