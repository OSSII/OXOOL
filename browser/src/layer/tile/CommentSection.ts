...
export class Comment extends CanvasSectionObject {
	...
	constructor (data: any, options: any, commentListSectionPointer: oxool.CommentSection) {
	...
	public onInitialize (): void {
		...
		if (this.sectionProperties.data.trackchange) {
			this.sectionProperties.captionNode = L.DomUtil.create('div', 'oxool-annotation-caption', this.sectionProperties.wrapper);
			this.sectionProperties.captionText = L.DomUtil.create('div', '', this.sectionProperties.captionNode);
		}

		this.sectionProperties.contentNode = L.DomUtil.create('div', 'oxool-annotation-content oxool-dont-break', this.sectionProperties.wrapper);
		this.sectionProperties.contentNode.id = 'annotation-content-area-' + this.sectionProperties.data.id;
		this.sectionProperties.nodeModify = L.DomUtil.create('div', 'oxool-annotation-edit' + ' modify-annotation', this.sectionProperties.wrapper);
		this.sectionProperties.nodeModifyText = L.DomUtil.create('textarea', 'oxool-annotation-textarea', this.sectionProperties.nodeModify);
		...
		this.sectionProperties.nodeReply = L.DomUtil.create('div', 'oxool-annotation-edit' + ' reply-annotation', this.sectionProperties.wrapper);
		this.sectionProperties.nodeReplyText = L.DomUtil.create('textarea', 'oxool-annotation-textarea', this.sectionProperties.nodeReply);
	...
	private createContainerAndWrapper (): void {
		...
		this.sectionProperties.container = L.DomUtil.create('div', 'oxool-annotation' + (isRTL ? ' rtl' : ''));
		...
		if (this.sectionProperties.data.trackchange) {
			this.sectionProperties.wrapper = L.DomUtil.create('div', 'oxool-annotation-redline-content-wrapper' + mobileClass, this.sectionProperties.container);
		} else {
			this.sectionProperties.wrapper = L.DomUtil.create('div', 'oxool-annotation-content-wrapper' + mobileClass, this.sectionProperties.container);
		}
		...
		if (oxool.CommentSection.autoSavedComment)
			this.sectionProperties.container.style.visibility = 'hidden';
	}
	...
	private createAuthorTable (): void {
		this.sectionProperties.author = L.DomUtil.create('table', 'oxool-annotation-table', this.sectionProperties.wrapper);

		var tbody = L.DomUtil.create('tbody', '', this.sectionProperties.author);
		var rowResolved = L.DomUtil.create('tr', '', tbody);
		var tdResolved = L.DomUtil.create('td', 'oxool-annotation-resolved', rowResolved);
		var pResolved = L.DomUtil.create('div', 'oxool-annotation-content-resolved', tdResolved);
		...
		var tdImg = L.DomUtil.create('td', 'oxool-annotation-img', tr);
		var tdAuthor = L.DomUtil.create('td', 'oxool-annotation-author', tr);
		...
		if (this.sectionProperties.docLayer._docType === 'text') {
			this.sectionProperties.replyCountNode = L.DomUtil.create('div', 'oxool-annotation-reply-count-collapsed', tdImg);
		...
		this.sectionProperties.contentAuthor = L.DomUtil.create('div', 'oxool-annotation-content-author', tdAuthor);
		this.sectionProperties.contentDate = L.DomUtil.create('div', 'oxool-annotation-date', tdAuthor);
		this.sectionProperties.autoSave = L.DomUtil.create('div', 'oxool-annotation-autosavelabel', tdAuthor);
	}

	private createMenu (): void {
		var tdMenu = L.DomUtil.create('td', 'oxool-annotation-menubar', this.sectionProperties.authorRow);
		this.sectionProperties.menu = L.DomUtil.create('div', this.sectionProperties.data.trackchange ? 'oxool-annotation-menu-redline' : 'oxool-annotation-menu', tdMenu);
	...
	private createTrackChangeButtons (): void {
		var tdAccept = L.DomUtil.create('td', 'oxool-annotation-menubar', this.sectionProperties.authorRow);
		var acceptButton = this.sectionProperties.acceptButton = L.DomUtil.create('button', 'oxool-redline-accept-button', tdAccept);

		var tdReject = L.DomUtil.create('td', 'oxool-annotation-menubar', this.sectionProperties.authorRow);
		var rejectButton = this.sectionProperties.rejectButton = L.DomUtil.create('button', 'oxool-redline-reject-button', tdReject);
	...
	public highlight (): void {
		if (this.sectionProperties.docLayer._docType === 'text') {
			...
			(this.containerObject.getSectionWithName(L.CSections.Scroll.name) as oxool.ScrollSection).onScrollTo({x: x, y: y});
		}
		else if (this.sectionProperties.docLayer._docType === 'spreadsheet') {
	...
	*/
	...
	private showWriter() {
		...
		if (this.sectionProperties.data.resolved === 'false' || this.sectionProperties.commentListSection.sectionProperties.showResolved)
			L.DomUtil.addClass(this.sectionProperties.container, 'oxool-annotation-collapsed-show');
	...
	public show(): void {
		...
		if (oxool.CommentSection.commentWasAutoAdded)
			return;
		if (this.sectionProperties.docLayer._docType === 'text')
			this.showWriter();
		else if (this.sectionProperties.docLayer._docType === 'presentation' || this.sectionProperties.docLayer._docType === 'drawing')
	...
	private hideWriter() {
		...
		L.DomUtil.removeClass(this.sectionProperties.container, 'oxool-annotation-collapsed-show');
	}
	...
	public isAutoSaved (): boolean {
		var autoSavedComment = oxool.CommentSection.autoSavedComment;
		if (!autoSavedComment)
			return false;
	...
	public handleReplyCommentButton (e: any): void {
		oxool.CommentSection.autoSavedComment = null;
		oxool.CommentSection.commentWasAutoAdded = false;
		this.textAreaInput();
		this.onReplyClick(e);
	}
	...
	public onLostFocus (e: any): void {
		if (!this.sectionProperties.isRemoved) {
			...
			if (this.sectionProperties.contentText.origText !== this.sectionProperties.nodeModifyText.value) {
				...
				oxool.CommentSection.autoSavedComment = this;
				this.onSaveComment(e);
			}
...
app.definitions.Comment = oxool.Comment;
