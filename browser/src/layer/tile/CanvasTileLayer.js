...
L.CanvasTileLayer = L.Layer.extend({
	...
	initialize: function (options) {
		...
		// Queue of tiles which were GC'd earlier than oxoolwsd expected
	...
	_onCellAddressMsg: function (textMsg) {
		...
		// message is received from oxoolwsd, *then* a 'celladdress' message.
	...
	_onCellFormulaMsg: function (textMsg) {
		// When a 'cellformula' message from oxoolwsd is received,
		// store the text contents of the cell, but don't push
		// them to the clipboard container (yet).
		// This is done because oxoolwsd will send several 'cellformula'
		...
		if (this._map['wopi'].DownloadAsPostMessage) {
			...
			if (this._map.options.print === false || L.Browser.cypressTest) {
				...
				if ('processOxoolUrl' in window) {
					url = window.processOxoolUrl({ url: url, type: 'print' });
				}
			...
			if ('processOxoolUrl' in window) {
				url = window.processOxoolUrl({ url: url, type: 'export' });
			}
	...
	// Given a character code and a UNO keycode, send a "key" message to oxoolwsd.
	//
	...
	// addition of sending messages to oxoolwsd, they move the cell cursor around.
	...
	_addDebugTools: function () {
		...
		*/
		...
		this._addDebugTool({
			...
			category: 'Functionality',
			startsOn: !!window.oxoolParams.get('randomUser'),
			onAdd: function () {
		...
		if (this.isCalc()) {
			this._addDebugTool({
				...
				startsOn: false,
				onAdd: function () {
					self._debugAutomatedUserAddTask(this.name, L.bind(self._debugAutomatedUserResizeRowsColumns, self));
				},
				onRemove: function () {
					self._debugAutomatedUserRemoveTask(this.name);
				},
		...
		if (this.isCalc()) {
			this._addDebugTool({
				...
				startsOn: false,
				onAdd: function () {
					self._debugAutomatedUserAddTask(this.name, L.bind(self._debugAutomatedUserDeleteRowsColumns, self));
				},
				onRemove: function () {
					self._debugAutomatedUserRemoveTask(this.name);
				},
	...
	_debugAutomatedUserTypeCellFormula: function (phase) {
		...
		switch (phase) {
			case 0:
				...
				var pos = this._latLngToTwips(this._map.getCenter());
	...
	_debugAutomatedUserResizeRowsColumns: function (phase) {
		var waitTime = 0;
		switch (phase) {
			case 0:
	...
	_debugAutomatedUserDeleteRowsColumns: function (phase) {
		var waitTime = 0;
		switch (phase) {
			case 0:
	...
	onAdd: function (map) {
		...
		map.fire('statusindicator', {statusType: 'oxoolloaded'});
	...
	// Fix for oxool#5876 allow immediate reuse of canvas context memory
	...
	_onTileMsg: function (textMsg, img) {
		...
		// keyframe content in JS but oxoolwsd still thinks we have
