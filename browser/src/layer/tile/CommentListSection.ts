...
export class CommentSection extends CanvasSectionObject {
	...
	static autoSavedComment: oxool.Comment;
	...
	public reply (annotation: any): void {
		...
		if ((<any>window).mode.isMobile()) {
			...
			if (this.sectionProperties.docLayer._docType === 'presentation' || this.sectionProperties.docLayer._docType === 'drawing') {
				...
				var replyAnnotationSection = new oxool.Comment(replyAnnotation, replyAnnotation.id === 'new' ? {noMenu: true} : {}, this);
	...
	public add (comment: any): oxool.Comment {
		var annotation = new oxool.Comment(comment, comment.id === 'new' ? {noMenu: true} : {}, this);
	...
	public onACKComment (obj: any): void {
		...
		if (action === 'Add') {
			...
			if (_redlined.sectionProperties.data.layoutStatus === CommentLayoutStatus.INSERTED) {
	...
	public importComments (commentList: any): void {
		...
		if (commentList.length > 0) {
			for (var i = 0; i < commentList.length; i++) {
				...
				var commentSection = new oxool.Comment(comment, {}, this);
	...
	public importChanges (changesList: any): void {
		...
		if (changesList.length > 0) {
			for (var i = 0; i < changesList.length; i++) {
				...
				var commentSection = new oxool.Comment(changeComment, {}, this);
				if (!this.containerObject.addSection(commentSection))
					continue;
				this.sectionProperties.commentList.push(commentSection);
			}
...
app.definitions.CommentSection = oxool.CommentSection;
