...
interface SectionCallbacks {
	...
	onResize?: () => void;
	onDraw?: (frameCount?: number, elapsedTime?: number, subsetBounds?: oxool.Bounds) => void;
...
class CanvasSectionContainer {
	...
	*/
	...
	private drawSections (frameCount: number = null, elapsedTime: number = null, tileSubset: Set<any> = null) {
		...
		var subsetBounds: oxool.Bounds = null;
		// if there is a tileSubset we only want to draw the miniumum region of its bounds
		if (tileSubset) {
			var tileSection = this.getSectionWithName(L.CSections.Tiles.name) as oxool.TilesSection;
