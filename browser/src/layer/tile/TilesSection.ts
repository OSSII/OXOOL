...
export class TilesSection extends CanvasSectionObject {
	...
	public getSubsetBounds(canvasCtx: CanvasRenderingContext2D, tileSubset: Set<any>): oxool.Bounds {
		...
		for (const coords of tileSubset) {
			...
			if (bounds === undefined)
				bounds = new oxool.Bounds(topLeft, rightBottom);
			else {
	...
	public clipSubsetBounds(canvasCtx: CanvasRenderingContext2D, subsetBounds: oxool.Bounds): void {
	...
	public onDraw (frameCount: number = null, elapsedTime: number = null, subsetBounds: oxool.Bounds = null): void {
