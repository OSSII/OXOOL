...
export class HeaderInfo {
	...
	constructor(map: any, _isColumn: boolean) {
		...
		const sheetGeom = this._map._docLayer.sheetGeometry as oxool.SheetGeometry;
		this._dimGeom = this._isColumn ? sheetGeom.getColumnsGeometry() : sheetGeom.getRowsGeometry();
	}

	findXInCellSelections (cellSelections: oxool.Rectangle[], ordinate: number): boolean {
	...
	findYInCellSelections (cellSelections: oxool.Rectangle[], ordinate: number): boolean {
		for (let i = 0; i < cellSelections.length; i++) {
	...
	isHeaderEntryHighLighted (cellSelections: oxool.Rectangle[], ordinate: number): boolean {
	...
	update(section: CanvasSectionObject): void {
		const cellSelections: oxool.Rectangle[] = this._map._docLayer._cellSelections;
		let currentIndex = -1;
...
L.Control.Header = oxool.Header;
L.Control.Header.HeaderInfo = oxool.HeaderInfo;
