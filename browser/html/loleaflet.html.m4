...
// EMSCRIPTENAPP) for a "WASM app". What we want is that the same loleaflet.html page adapts on demand to
// instead run locally using WASM, if the connection to the OXOOL server breaks. (And then
// re-connects to the OXOOL server when possible.)
...
m4_ifelse(MOBILEAPP,[true],
  [   window.ThisIsAMobileApp = true;
   ...
   window.HelpFile = String.raw`m4_syscmd([cat html/oxool-help.html])`;
...
m4_ifelse(EMSCRIPTENAPP,[true],
  [   window.ThisIsTheEmscriptenApp = true;
   window.postMobileMessage = function(msg) { app.HandleOXOOLMessage(app.AllocateUTF8(msg)); };
   window.postMobileError   = function(msg) { console.log('OXOOL Error: ' + msg); };
   window.postMobileDebug   = function(msg) { console.log('OXOOL Debug: ' + msg); };],
...
m4_ifelse(BUNDLE,[],
  ...
  m4_foreachq([fileCSS],[OXOOL_CSS],[<link rel="stylesheet" href="][m4_ifelse(MOBILEAPP,[],[%SERVICE_ROOT%/browser/%VERSION%/])][fileCSS" />
]),
      ...
      <div id="about-dialog-container">
        ...
        <div id="about-dialog-info-container">
          <div id="about-dialog-info">
            ...
            <div style="margin-inline-end: auto;"><div id="oxoolwsd-version" dir="ltr"></div></div>
...
m4_ifelse(MOBILEAPP,[true],
     ...
     [window.host = '%HOST%';
      ...
      window.oxoolLogging = '%BROWSER_LOGGING%';
      window.oxoolwsdVersion = '%OXOOLWSD_VERSION%';
...
// "dir" parameter
m4_ifelse(IOSAPP,[true],
     [document.dir = window.oxoolParams.get('dir');])

m4_ifelse(IOSAPP,[true],
     [window.userInterfaceMode = window.oxoolParams.get('userinterfacemode');])

m4_ifelse(ANDROIDAPP,[true],
     [window.userInterfaceMode = window.oxoolParams.get('userinterfacemode');])

m4_ifelse(ANDROIDAPP,[true],
     [var darkTheme = window.oxoolParams.get('darkTheme');
