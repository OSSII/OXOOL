...
var AdminSocketSettings = AdminSocketBase.extend({
	...
	onSocketMessage: function(e) {
		...
		else if (textMsg.startsWith('oxoolserver ')) {
			// This must be the first message, unless we reconnect.
			var oxoolwsdVersionObj = JSON.parse(textMsg.substring(textMsg.indexOf('{')));
			var h = oxoolwsdVersionObj.Hash;
			if (parseInt(h,16).toString(16) === h.toLowerCase().replace(/^0+/, '')) {
				h = '<a target="_blank" href="https://github.com/CollaboraOnline/online/commits/' + h + '">' + h + '</a>';
				$('#oxoolwsd-version').html(oxoolwsdVersionObj.Version + ' (git hash: ' + h + ')');
			}
			else {
				$('#oxoolwsd-version').text(oxoolwsdVersionObj.Version);
			}
			$('#oxoolwsd-buildconfig').html(oxoolwsdVersionObj.BuildConfig);
		}
