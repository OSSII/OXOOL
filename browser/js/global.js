...
(function (global) {
	...
	global.setLogging = function(doLogging)
	{
		...
		if (!doLogging) {
			...
			for (var i = 0; i < loggingMethods.length; i++) {
				...
				(function(method) {
					global.app.console[method] = function logWithOxool() {
	...
	var chromebook = global.ThisIsTheAndroidApp && global.OXOOLMessageHandler.isChromeOS();
	...
	global.L.Params = {
		...
		closeButtonEnabled: global.oxoolParams.get('closebutton'),

		/// Shows revision history file menu option
		revHistoryEnabled: global.oxoolParams.get('revisionhistory'),
	};
	...
	global.memo = {
		...
		_getId: function(obj) {
			...
			if (!('_oxoolMemoId' in obj)) {
				obj['_oxoolMemoId'] = ++global.memo._lastId;
			}
			return obj._oxoolMemoId;
		},
	...
	global.createWebSocket = function(uri) {
		if ('processOxoolUrl' in global) {
			uri = global.processOxoolUrl({ url: uri, type: 'ws' });
		}
	...
	global.wopiSrc = global.oxoolParams.get('WOPISrc');
	if (global.wopiSrc != '') {
	...
	if (global.ThisIsAMobileApp) {
		...
		var websocketURI = global.makeWsUrlWopiSrc('/oxool/', global.docURL + docParamsPart);
		try {
	...
	var isRandomUser = global.oxoolParams.get('randomUser');
	if (isRandomUser) {
		...
		global.oxoolParams.set('lang', randomUserLang);
		global.oxoolParams.set('debug',true);
	}
	...
	if (global.socket && global.socket.readyState !== 3) {
		global.socket.onopen = function () {
			if (global.socket.readyState === 1) {
				...
				var timestamp = encodeURIComponent(global.oxoolParams.get('timestamp'));
				var msg = 'load url=' + encodeURIComponent(global.docURL);

				var now0 = Date.now();
				var now1 = performance.now();
				var now2 = Date.now();
				global.socket.send('oxoolclient ' + ProtocolVersionNumber + ' ' + ((now0 + now2) / 2) + ' ' + now1);
