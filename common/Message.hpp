...
public:
    ...
    {
        return OXOOLProtocol::getTokenInteger(_tokens, name, value);
    }

    /// Return the abbreviated message for logging purposes.
    std::string abbr() const {
        return _id + ' ' + OXOOLProtocol::getAbbreviatedMessage(_data.data(), _data.size());
    }
    ...
    void rewriteDataBody(const std::function<bool (std::vector<char> &)>& func)
    {
        ...
        {
            // Check - just the body.
            assert(_firstLine == OXOOLProtocol::getFirstLine(_data.data(), _data.size()));
...
private:
    ...
    void assignFirstLineIfEmpty()
    {
        if(_firstLine.empty())
        {
            _firstLine = OXOOLProtocol::getFirstLine(_data.data(), _data.size());
        }
    }
    ...
    {
        std::string forward = OXOOLProtocol::getFirstToken(buffer, length);
