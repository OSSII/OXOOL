...
/// we need to make sure that we use OXOOLWebSocket all over the place.
...
/// larger code changes.
class OXOOLWebSocket : public Poco::Net::WebSocket
{
public:
    OXOOLWebSocket(const Socket& socket) :
        Poco::Net::WebSocket(socket)
    {
    }

    OXOOLWebSocket(Poco::Net::HTTPClientSession& cs,
                  ...
                  const std::string& testname = std::string())
    {
        ...
        {
            TST_LOG("WARN: OXOOLWebSocket::shutdown: Exception: "
                    << exc.displayText()
...
#undef CASE
        ...
        if (length > 0 &&
            ...
            result << ": '" << OXOOLProtocol::getAbbreviatedMessage(message, length) << '\'';
        return result.str();
    }
};
