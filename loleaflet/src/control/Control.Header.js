...
L.Control.Header = L.Class.extend({
	...
	onMouseMove: function (point, dragDistance) {
		if (!this.containerObject.draggingSomething) { // If we are not dragging anything.
			...
			/*
			  ...
			  模擬滑鼠連續選取欄或列的行為
			*/
			if (!window.mode.isMobile() && !this._hitResizeArea && this._mouseOverEntry) {
				var origPosition = this._mouseOverEntry.index; // 紀錄起始編號
				var result = this._entryAtPoint(point);
				if (result) {
					// TODO: 還可再做強化，檢查新位置若和之前一樣，
					// 就不需要再執行 _selectColumn 或 _selectRow
					var newPosition = result.entry.index; // 紀錄新的編號
					if (newPosition !== origPosition) {
						var modifier = UNOModifier.SHIFT;
						if (this._isColumn) {
							this._selectColumn(newPosition, modifier);
						} else {
							this._selectRow(newPosition, modifier);
						}
					}
				}
			}
	...
	onMouseDown: function (point, e) {
		...
		if (this._mouseOverEntry) {
			var position = this._mouseOverEntry.index;
			...
			if (this._isColumn) {
				this._selectColumn(position, modifier);
			} else {
				this._selectRow(position, modifier);
			}
		}
