...
class CanvasSectionObject {
	...
	onDrawArea: (area?: oxool.Bounds, paneTopLeft?: oxool.Point, canvasContext?: CanvasRenderingContext2D) => void;
	...
	*/
	...
	private calculateSectionInitialPosition(section: CanvasSectionObject, index: number): number {
		if (typeof section.anchor[index] === 'string' || section.anchor[index].length === 1) {
			...
			if (section.anchor[index].length % 2 === 0) { // eslint-disable-line no-lonely-if
				window.app.console.error('Section: ' + section.name + '. Wrong anchor definition.');
				return 0;
			}
			else {
				...
				if (targetSection) {
					...
					if (!targetSection.isLocated) {
						window.app.console.error('Section: ' + section.name + '. Target section for anchor should be located before this section.'
	...
	private checkNewSectionName (options: any) {
		if (options.name !== undefined && typeof options.name === 'string' && options.name.trim() !== '') {
			if (this.doesSectionExist(options.name)) {
				window.app.console.error('There is a section with the same name. Use doesSectionExist for existancy checks.');
				return false;
			}
			else if (this.arrayIncludes(['top', 'left', 'bottom', 'right'], options.name.trim())) {
				window.app.console.error('"top", "left", "bottom" and "right" words are reserved. Choose another name for the section.');
				return false;
			}
			else {
				return true;
			...
			window.app.console.error('Sections should have a "name" property.');
			return false;
		}
	}
	...
	private checkSectionProperties (options: any) {
		if (
			...
			window.app.console.error('Section has missing properties. See "checkSectionProperties" function.');
			return false;
		}
		else {
