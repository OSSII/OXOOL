...
L.TextInput = L.Layer.extend({
	initialize: function() {
		...
		this._isAndroidWebkit = (L.Browser.android && L.Browser.webkit);

		this._preSpaceChar = ' ';
		// Might need to be \xa0 in some legacy browsers ?
		if (this._isAndroidWebkit) {
	...
	_onCompositionStart: function(/*ev*/) {
		...
		// 由於無法阻止組字事件，所以直接設定失焦，阻止繼續組字
		if (!this._isComposing) {
			this._textArea.blur();
		} else {
			this._textArea.value = ''; // 清空組字區
			// 將組字輸入區設為可見
