 ...
 */
/*
 * L.Control.EnhancedStatusBar is responsible for the status bar at the bottom of the document.
 */

/* global $ w2ui _ _UNO */
L.Control.EnhancedStatusBar = L.Control.extend({
	...
	onDocLayerInit: function () {
		...
		var insertMode = function (e) {
			...
			this.html = '<div id="InsertMode" class="oxool-font insert-mode-' + state + '" title="' + _('Entering text mode') + '">' + mode + '</div>';
			...
			case 'spreadsheet':
				{
					statusbar.insert('left', [
						{ type: 'break', id: 'break1' },
						{
							...
							onClick: function () {
								map.sendUnoCommand('.uno:JumpToTable');
							},
							stateChange: function (e) {
								map.simpleStateChecker(e, this);
								this.text = e.value();
								statusbar.refresh(this.id);
							}
						},
						...
						{
							type: 'html', id: 'RowColSelCount', mobile: false,
							html: '',
							stateChange: function (e) {
								map.simpleStateChecker(e, this);
								var value = e.hasValue() ? e.value() : '';
								this.html = '<div title="' + _('Selected range of cells') + '">' + value + '</div>';
								statusbar.refresh(this.id);
							}
						},
						{ type: 'break', id: 'break3', mobile: false },
						{
							type: 'button', text: '', id: 'StatusPageStyle', hint: _('Page Style') + '(' + _('Click to open the styles dialog') + ')',
						...
						{ type: 'break', id: 'break8', mobile: false, tablet: false },
						{
							...
							stateChange: function (e) {
								var value = e.hasValue() ? e.value() : '';
								map.simpleStateChecker(e, this);
								this.html = '<div title="' + _('Choice of functions') + '">' + value + '</div>';
								statusbar.refresh(this.id);
							}
						},
						{
			...
			case 'text':
				{
					statusbar.insert('left', [
						{ type: 'break', id: 'break1' },
						{
							...
							onClick: function () {
								map.sendUnoCommand('.uno:GotoPage');
							},
							stateChange: function (e) {
								map.simpleStateChecker(e, this);
								this.text = e.value();
						...
						{
							type: 'button', text: '', id: 'PageStyleName', hint: _('Page Style') + '(' + _('Click to open the styles dialog') + ')',
							mobile: false, tablet: false,
							onClick: function () {
			...
			case 'presentation':
				{
					statusbar.insert('left', [
						{ type: 'break', id: 'break1' },
						{
							...
							stateChange: function (e) {
								map.simpleStateChecker(e, this);
								this.html = '<div class="oxool-font" title="' + _('Number of Slides') + '">' + (e.hasValue() ? e.value() : '') + '</div>';
								statusbar.refresh(this.id);
							}
						},
						{ type: 'break', id: 'break-Context', mobile: false },
						{
							type: 'html', id: 'Context', mobile: false,
							html: '',
							stateChange: function (e) {
								map.simpleStateChecker(e, this);
								this.html = '<div class="oxool-font">' + (e.hasValue() ? e.value() : '') + '</div>';
								statusbar.refresh(this.id);
							}
						},
			...
			case 'drawing':
				{
					statusbar.insert('left', [
						...
						{
							type: 'html', id: 'PageStatus',
							html: '<div id="PageStatus" class="oxool-font" title="' + _('Number of Pages') + '">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp</div>'
						},
						{ type: 'break', id: 'break2', mobile: false, tablet: false },
						{
	...
	_getPermissionModeHtml: function (isReadOnly, canUserWrite) {
		var permissionModeDiv = '<div id="PermissionMode" class="oxool-font ';
		if (isReadOnly && !canUserWrite) {
			permissionModeDiv += ' status-readonly-mode" title="' + _('Permission Mode') + '"> ' + _('Read-only') + ' </div>';
		} else if (isReadOnly && canUserWrite) {
			permissionModeDiv += ' status-readonly-transient-mode" style="display: none;"></div>';
		} else {
			permissionModeDiv += ' status-edit-mode" title="' + _('Permission Mode') + '"> ' + _('Edit') + ' </div>';
		}
...
L.control.statusBar = function () {
	return new L.Control.EnhancedStatusBar();
};
