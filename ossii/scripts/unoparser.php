...
foreach ($unoCcommandFiles as $xcu => $data)
{
	...
	{
		echo 'ERROR : '.$xcuFile.' not found!'.chr(10);
		exit(255);
	}
}

$unojs = $oxoolSourcePath . '/ossii/browser/js/unocommands.js';
...
if ($handle = opendir($loSourcePath.$localePath))
{
	...
	$targetPath = $oxoolSourcePath.'/browser/l10n/uno';
	while (false !== ($entry = readdir($handle)))
	{
		if ($entry != '.' && $entry != '..')
		{
			...
			if (is_file($UIpo))
			{
				...
				echo ($retuenVar == 0 ? 'OK' : 'Fail').chr(10);
			}
		}
	}
	closedir($handle);
