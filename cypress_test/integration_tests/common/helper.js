...
function checkOxoolFrameGlobal() {
	cy.log('checkOxoolFrameGlobal - start.');
	cy.get('#oxoolframe').its('0.contentDocument').should('exist').its('body').should('not.be.undefined');
	cy.log('checkOxoolFrameGlobal - end.');
}

function checkFirstOxoolFrameGlobal() {
	cy.log('checkFirstOxoolFrameGlobal - start.');
	cy.get('#iframe1').its('0.contentDocument').should('exist').its('body').should('not.be.undefined');
	cy.log('checkFirstOxoolFrameGlobal - end.');
}

function checkSecondOxoolFrameGlobal() {
	cy.log('checkSecondOxoolFrameGlobal - start.');
	cy.get('#iframe2').its('0.contentDocument').should('exist').its('body').should('not.be.undefined');
	cy.log('checkSecondOxoolFrameGlobal - end.');
}
...
function checkIfDocIsLoaded(isMultiUser) {
	...
	if (isMultiUser) {
		...
		checkFirstOxoolFrameGlobal();
		checkSecondOxoolFrameGlobal();
		...
		cy.frameLoaded('#oxoolframe');
		checkOxoolFrameGlobal();
		cy.cSetActiveFrame('#oxoolframe');
		documentChecks();
	}
...
function getOxoolFrameWindow() {
	return cy.get('#oxoolframe')
		.its('0.contentWindow')
		.should('exist');
}
...
module.exports.getOxoolFrameWindow = getOxoolFrameWindow;
