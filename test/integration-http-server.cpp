...
class HTTPServerTest : public CPPUNIT_NS::TestFixture
{
    ...
    CPPUNIT_TEST(testOxoolPostPoco);
...
public:
    ...
    void setUp()
    {
        ...
        helpers::testCountHowManyOxoolkits();
        helpers::resetTestStartTime();
    }
    ...
    {
        helpers::resetTestStartTime();
        helpers::testNoExtraOxoolKitsLeft();
        helpers::resetTestStartTime();
    }
...
void HTTPServerTest::testOxoolPostPoco()
{
    constexpr auto testname = __func__;
...
void HTTPServerTest::testOxoolPost()
{
    ...
    const auto pathAndQuery = "/browser/dist/"
                              "loleaflet.html?WOPISrc=https%3A%2F%2Flocalhost%2Fnextcloud%2Findex.php%"
    ...
    fprintf(stderr, "%s\n", html.c_str());
    LOK_ASSERT(html.find(_uri.getHost()) != std::string::npos);
    LOK_ASSERT(html.find("window.versionPath = '" OXOOLWSD_VERSION_HASH "';") != std::string::npos);
    LOK_ASSERT(html.find("window.oxoolwsdVersion = '" OXOOLWSD_VERSION "';") != std::string::npos);
