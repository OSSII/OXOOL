                ...
                _dosThread = std::thread(
                    [this]
                    {
                        for (;;)
                        {
                            ...
                            LOG_TST("Connecting test client to OXOOL (#"
                                    << _count << " connection): /oxool/" << wopiSrc << "/ws");

                            Poco::URI uri(helpers::getTestServerURI());
                            const std::string documentURL = "/oxool/" + wopiSrc + "/ws";
