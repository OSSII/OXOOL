...
void AdminSocketHandler::handleMessage(const std::vector<char> &payload)
{
            ...
            if (pids.find(pid) != pids.end())
            {
                if (Admin::instance().logAdminAction())
                {
                    LOG_ANY("Admin request to kill document ["
                            << OXOOLWSD::anonymizeUrl(model.getFilename(pid)) << "] with pid ["
    ...
    else if (tokens.equals(0, "migrate") && tokens.size() > 1)
    {
        ...
        if (!dockey.empty() && !routeToken.empty() && !serverId.empty())
        {
            ...
            if (SigUtil::getShutdownRequestFlag())
                OXOOLWSD::setMigrationMsgReceived(dockey);
        }
        else
        {
    ...
    {
        // 建立指向 sendTextFrame 的 callback
        std::function<void(const std::string&, bool)> sendTextMessage =
        [this](const std::string& message, bool flush)
        {
            if (_isAuthenticated)
            {
        ...
        // 交給模組處理
        OxOOL::handleAdminMessage(sendTextMessage, tokens);
    }
}
    ...
    , _cleanupIntervalMs(DefStatsIntervalMs * 10)
{
    ...
    // as headroom. Otherwise, oxoolwsd might fail to
    ...
    const double memLimit = OXOOLWSD::getConfigValue<double>("memproportion", 0.0);
    if (memLimit > 0.0)
    {
...
void Admin::pollingThread()
{
    ...
    if (!OXOOLWSD::IndirectionServerEnabled)
        return;
    ...
    if (!controllerMonitorConnection)
    {
        ...
        OXOOLWSD::setAllMigrationMsgReceived();
        return;
    }
...
void Admin::connectToMonitorSync(const std::string &uri)
{
    ...
    if (OXOOLWSD::getConfigValue<bool>("admin_console.logging.monitor_connect", true))
    {
...
void Admin::sendMetrics(const std::shared_ptr<StreamSocket>& socket, const std::shared_ptr<Poco::Net::HTTPResponse>& response)
{
    ...
    bool showLog = OXOOLWSD::getConfigValue<bool>("admin_console.logging.metrics_fetch", true);
