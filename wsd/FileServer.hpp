...
class FileServerRequestHandler
{
    ...
    /// Construct a JSON to be accepted by the loleaflet.html from a list like
...
public:
    ...
    static void hstsHeaders([[maybe_unused]] http::Response& response)
    {
        ...
        {
            if (OXOOLWSD::getConfigValue<bool>("ssl.sts.enabled", false))
            {
                static const auto maxAge =
                    OXOOLWSD::getConfigValue<int>("ssl.sts.max_age", 31536000); // Default 1 year.
