    ...
    bool sendFrame(const std::string& data, bool binary = false, bool flush = false)
    {
        try
        {
            if (_ws)
            {
                LOG_TRC("Send to " << _name << " message: ["
                                   << OXOOLProtocol::getAbbreviatedMessage(data) << ']');
        ...
        catch (const std::exception& exc)
        {
            LOG_ERR("Failed to send " << _name << " [" << _pid << "] data [" <<
                    OXOOLProtocol::getAbbreviatedMessage(data) << "] due to: " << exc.what());
            throw;
        }

        LOG_WRN("No socket to " << _name << " to send [" << OXOOLProtocol::getAbbreviatedMessage(data) << ']');
        return false;
    }
...
class OXOOLWSD final : public Poco::Util::ServerApplication
{
public:
    OXOOLWSD();
    ~OXOOLWSD();
    ...
    {
#if ENABLE_SSL
        return !Util::isFuzzing() && OXOOLWSD::SSLEnabled.get();
#else
        return false;
#endif
    }
    ...
    {
#if ENABLE_SSL
        return !Util::isFuzzing() && OXOOLWSD::SSLTermination.get();
#else
        return false;
#endif
    }
    ...
    std::string getPathFromConfigWithFallback(const std::string& name, const std::string& fallbackName)
    {
        ...
        try
        {
            value = OXOOLWSD::getPathFromConfig(name);
        }
        catch (...)
        {
        }
        if (value.empty())
            return OXOOLWSD::getPathFromConfig(fallbackName);
        return value;
    }
...
protected:
    void initialize(Poco::Util::Application& self) override
    {
        ...
        {
            LOG_FTL("Failed to initialize OXOOLWSD: "
                    << ex.displayText()
        ...
        {
            LOG_FTL("Failed to initialize OXOOLWSD: " << ex.what());
            throw; // Nothing further to do.
        }
    }
