...
void WopiProxy::handleRequest(SocketPoll& poll, SocketDisposition& disposition)
{
    ...
    LOG_INF("Starting GET request handler for session [" << _id << "] on url ["
                                                         << OXOOLWSD::anonymizeUrl(url) << "].");

    LOG_INF("Sanitized URI [" << OXOOLWSD::anonymizeUrl(url) << "] to ["
    ...
    switch (storageType)
    {
        case StorageBase::StorageType::Unsupported:
            LOG_ERR("Unsupported URI [" << OXOOLWSD::anonymizeUrl(uriPublic.toString())
        ...
        case StorageBase::StorageType::FileSystem:
            LOG_INF("URI [" << OXOOLWSD::anonymizeUrl(uriPublic.toString()) << "] on docKey ["
...
{
    const std::string uriAnonym = OXOOLWSD::anonymizeUrl(uriPublic.toString());
         ...
         redirectLimit](const std::shared_ptr<http::Session>& session)
    {
        ...
        if (!fileUrl.empty())
        {
            const std::string fileUrlAnonym = OXOOLWSD::anonymizeUrl(fileUrl);
            try
            {
