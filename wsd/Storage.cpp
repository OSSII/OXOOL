...
void StorageBase::initialize()
{
    ...
    if (SSLEnabled || SSLAsScheme)
    {
        ...
        const bool sslDefaultVerification = caLocationEmpty ? false : OXOOLWSD::getConfigValue<bool>("ssl.ssl_verification", true);
        // Get storage.ssl.ssl_verification, and use sslDefaultVerification as fallback
        const bool sslVerification = OXOOLWSD::getConfigValue<bool>("storage.ssl.ssl_verification", sslDefaultVerification);
