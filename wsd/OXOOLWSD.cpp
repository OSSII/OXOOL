...
#define OXOOLWSD_TEST_HOST "localhost"

/* Default oxool UI used in the admin console URI */
#define OXOOLWSD_TEST_ADMIN_CONSOLE "/browser/dist/admin/admin.html"

/* Default oxool UI used in for monitoring URI */
#define OXOOLWSD_TEST_METRICS "/oxool/getMetrics"

/* Default oxool UI used in the start test URI */
#define OXOOLWSD_TEST_OXOOL_UI "/browser/" OXOOLWSD_VERSION_HASH "/debug.html"

/* Default document used in the start test URI */
#define OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_WRITER  "test/data/hello-world.odt"
#define OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_CALC    "test/data/hello-world.ods"
#define OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_IMPRESS "test/data/hello-world.odp"
#define OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_DRAW    "test/data/hello-world.odg"

/* Default ciphers used, when not specified otherwise */
#define DEFAULT_CIPHER_SET "ALL:!ADH:!LOW:!EXP:!MD5:@STRENGTH"

// This is the main source for the oxoolwsd program. OXOOL uses several oxoolwsd processes: one main
// parent process that listens on the TCP port and accepts connections from OXOOL clients, and a
...
std::string OXOOLWSD::BuyProductUrl;
std::string OXOOLWSD::LatestVersion;
std::mutex OXOOLWSD::FetchUpdateMutex;
std::mutex OXOOLWSD::RemoteConfigMutex;
#endif
...
std::atomic<unsigned> OXOOLWSD::NumConnections;
std::unordered_set<std::string> OXOOLWSD::EditFileExtensions;
std::unordered_set<std::string> OXOOLWSD::ViewWithCommentsFileExtensions;

#if MOBILEAPP

// Or can this be retrieved in some other way?
int OXOOLWSD::prisonerServerSocketFD;
...
inline void shutdownLimitReached(const std::shared_ptr<ProtocolHandlerInterface>& proto)
{
    ...
    const std::string error = Poco::format(PAYLOAD_UNAVAILABLE_LIMIT_REACHED, OXOOLWSD::MaxDocuments, OXOOLWSD::MaxConnections);
...
void OXOOLWSD::appendAllowedHostsFrom(LayeredConfiguration& conf, const std::string& root, std::vector<std::string>& allowed)
{
    for (size_t i = 0; ; ++i)
    {
...
void OXOOLWSD::alertAllUsersInternal(const std::string& msg)
{
...
void OXOOLWSD::alertUserInternal(const std::string& dockey, const std::string& msg)
{
    std::lock_guard<std::mutex> docBrokersLock(DocBrokersMutex);
...
void OXOOLWSD::writeTraceEventRecording(const char *data, std::size_t nbytes)
{
    static std::mutex traceEventFileMutex;

    std::unique_lock<std::mutex> lock(traceEventFileMutex);

    fwrite(data, nbytes, 1, OXOOLWSD::TraceEventFile);
}

void OXOOLWSD::writeTraceEventRecording(const std::string &recording)
{
...
void OXOOLWSD::checkSessionLimitsAndWarnClients()
{
    ...
    if (OXOOLWSD::MaxDocuments < 10000 &&
        (docBrokerCount > static_cast<ssize_t>(OXOOLWSD::MaxDocuments) || OXOOLWSD::NumConnections >= OXOOLWSD::MaxConnections))
    {
        const std::string info = Poco::format(PAYLOAD_INFO_LIMIT_REACHED, OXOOLWSD::MaxDocuments, OXOOLWSD::MaxConnections);
...
void OXOOLWSD::checkDiskSpaceAndWarnClients(const bool cacheLastCheck)
{
#if !MOBILEAPP
    try
    {
        ...
        if (!fs.empty())
        {
            ...
            OXOOLWSD::alertAllUsersInternal("error: cmd=internal kind=diskfull");
        }
    }
...
void cleanupDocBrokers()
{
    ...
    if (count != DocBrokers.size())
    {
        ...
        if (OXOOLWSD::SingleKit && DocBrokers.empty())
        {
...
static int forkChildren(const int number)
{
    ...
    if (number > 0)
    {
        OXOOLWSD::checkDiskSpaceAndWarnClients(false);

#ifdef KIT_IN_PROCESS
        forkLibreOfficeKit(OXOOLWSD::ChildRoot, OXOOLWSD::SysTemplate, OXOOLWSD::LoTemplate, number);
#else
        const std::string aMessage = "spawn " + std::to_string(number) + '\n';
        LOG_DBG("MasterToForKit: " << aMessage.substr(0, aMessage.length() - 1));
        OXOOLWSD::sendMessageToForKit(aMessage);
#endif
...
static bool prespawnChildren()
{
    ...
    return lock.try_lock() && (rebalanceChildren(OXOOLWSD::NumPreSpawnedChildren) > 0);
}
...
static size_t addNewChild(std::shared_ptr<ChildProcess> child)
{
    ...
    if (OXOOLWSD::IsBindMountingEnabled)
    {
...
#if MOBILEAPP
#ifndef IOS
std::mutex OXOOLWSD::lokit_main_mutex;
#endif
#endif
...
std::shared_ptr<ChildProcess> getNewChild_Blocks(unsigned mobileAppDocId)
{
    ...
    int numPreSpawn = OXOOLWSD::NumPreSpawnedChildren;
    ...
    std::thread([&]
                {
#ifndef IOS
                    std::lock_guard<std::mutex> lock(OXOOLWSD::lokit_main_mutex);
                    Util::setThreadName("lokit_main");
#else
                    Util::setThreadName("lokit_main_" + Util::encodeId(mobileAppDocId, 3));
#endif
                    // Ugly to have that static global OXOOLWSD::prisonerServerSocketFD, Otoh we know
                    // there is just one OXOOLWSD object. (Even in real Online.)
                    lokit_main(OXOOLWSD::prisonerServerSocketFD, OXOOLWSD::UserInterface, mobileAppDocId);
                }).detach();
#endif
...
public:
    ...
    virtual void handlePart(const MessageHeader& header, std::istream& stream) override
    {
        ...
        Path tempPath = Path::forDirectory(
            FileUtil::createRandomTmpDir(OXOOLWSD::ChildRoot + JailUtil::CHILDROOT_TMP_INCOMING_PATH)
            + '/');
        ...
        if (name == "document")
        {
            ...
            Path tempPath =
                Path::forDirectory(FileUtil::createRandomTmpDir(
                                       OXOOLWSD::ChildRoot + JailUtil::CHILDROOT_TMP_INCOMING_PATH) +
                                   '/');
...
inline std::string getLaunchBase(bool asAdmin = false)
{
    ...
    oss << "    ";
    oss << ((OXOOLWSD::isSSLEnabled() || OXOOLWSD::isSSLTermination()) ? "https://" : "http://");

    if (asAdmin)
    {
        auto user = OXOOLWSD::getConfigValue<std::string>("admin_console.username", "");
        auto passwd = OXOOLWSD::getConfigValue<std::string>("admin_console.password", "");
...
inline std::string getLaunchURI(const std::string &document)
{
    ...
    oss << getLaunchBase();
    oss << OXOOLWSD::ServiceRoot;
    oss << OXOOLWSD_TEST_OXOOL_UI;
    oss << "?file_path=";
    oss << DEBUG_ABSSRCDIR "/";
    oss << document;
                    ...
                    const std::string &errorMsg)
{
    ...
    std::string errorMsgFormatted = OXOOLProtocol::getAbbreviatedMessage(errorMsg);
...
bool OXOOLWSD::UnattendedRun = false;
bool OXOOLWSD::SignalParent = false;
bool OXOOLWSD::UseEnvVarOptions = false;
std::string OXOOLWSD::RouteToken;
#if ENABLE_DEBUG
...
std::string OXOOLWSD::CleanupChildRoot;
std::string OXOOLWSD::ChildRoot;
std::string OXOOLWSD::ServerName;
std::string OXOOLWSD::FileServerRoot;
...
bool OXOOLWSD::AnonymizeUserData = false;
bool OXOOLWSD::CheckOxoolUser = true;
bool OXOOLWSD::CleanupOnly = false; //< If we should cleanup and exit.
bool OXOOLWSD::IsProxyPrefixEnabled = false;
#if ENABLE_SSL
Util::RuntimeConstant<bool> OXOOLWSD::SSLEnabled;
Util::RuntimeConstant<bool> OXOOLWSD::SSLTermination;
#endif
unsigned OXOOLWSD::MaxConnections;
unsigned OXOOLWSD::MaxDocuments;
std::string OXOOLWSD::OverrideWatermark;
...
unsigned int OXOOLWSD::NumPreSpawnedChildren = 0;
...
void InotifySocket::handlePoll(SocketDisposition & /* disposition */, std::chrono::steady_clock::time_point /* now */, int /* events */)
{
    ...
    while (true)
    {
        ...
        for (char* ptr = buf; ptr < buf + len; ptr += sizeof(struct inotify_event) + event->len)
        {
            ...
            LOG_WRN("InotifyPoll - Config file " << event->name << " was modified, stopping OXOOLWSD");
            SigUtil::requestShutdown();
        }
    }
}
...
class OXOOLWSDServer;
static std::unique_ptr<OXOOLWSDServer> Server;
...
{
    LOG_TRC("ForKitProcWSHandler: handling incoming [" << OXOOLProtocol::getAbbreviatedMessage(&data[0], data.size()) << "].");
    const std::string firstLine = OXOOLProtocol::getFirstLine(&data[0], data.size());
...
public:
    ...
    void handleOptions(const Poco::JSON::Object::Ptr& remoteJson)
    {
        try
        {
            ...
            {
                std::lock_guard<std::mutex> lock(OXOOLWSD::RemoteConfigMutex);
                OXOOLWSD::BuyProductUrl = buyProductUri.toString();
            }
        }
...
private:
    ...
    bool finishDownload(const std::string& uri, const std::shared_ptr<const http::Response> httpResponse)
    {
        ...
        fontFile = OXOOLWSD::TmpFontDir + "/" + Util::encodeId(Util::rng::getNext()) + ".ttf";
        ...
        OXOOLWSD::sendMessageToForKit("addfont " + fontFile);
    ...
    void restartForKitAndReDownloadConfigFile()
    {
        ...
        _eTagValue = "";
        OXOOLWSD::sendMessageToForKit("exit");
    }
...
void OXOOLWSD::innerInitialize(Application& self)
{
#if !MOBILEAPP
    if (geteuid() == 0 && CheckOxoolUser)
    {
    ...
    static const std::map<std::string, std::string> DefAppConfig = {
        ...
        { "ssl.key_file_path", OXOOLWSD_CONFIGDIR "/key.pem" },
    ...
    Poco::Path configPath("oxoolwsd.xml");
    if (Application::findFile(configPath))
        loadConfiguration(configPath.toString(), PRIO_DEFAULT);
    else
    {
        // Fallback to the OXOOLWSD_CONFIGDIR or --config-file path.
    ...
    setenv("OXOOL_LOGLEVEL", LogLevel.c_str(), true);
    ...
    if (logToFile)
    {
        ...
        {
            setenv("OXOOL_LOGFILE", "1", true);
            setenv("OXOOL_LOGFILENAME", it->second.c_str(), true);
    ...
    setenv("OXOOL_LOGLEVEL_STARTUP", LogLevelStartup.c_str(), true);
    ...
    if (AnonymizeUserData)
    {
        ...
        setenv("OXOOL_ANONYMIZATION_SALT", anonymizationSaltStr.c_str(), true);
    }
    FileUtil::setUrlAnonymization(AnonymizeUserData, anonymizationSalt);

    {
        bool enableWebsocketURP =
            OXOOLWSD::getConfigValue<bool>("security.enable_websocket_urp", false);
...
#if ENABLE_SSL
    OXOOLWSD::SSLEnabled.set(getConfigValue<bool>(conf, "ssl.enable", true));
    OXOOLWSD::SSLTermination.set(getConfigValue<bool>(conf, "ssl.termination", true));
#endif

    LOG_INF("SSL support: SSL is " << (OXOOLWSD::isSSLEnabled() ? "enabled." : "disabled."));
    LOG_INF("SSL support: termination is " << (OXOOLWSD::isSSLTermination() ? "enabled." : "disabled."));
    ...
    setenv("OXOOL_CONFIG", oss.str().c_str(), true);
    ...
    {
        OXOOLWSD::MaxConnections = 20;
        OXOOLWSD::MaxDocuments = 10;
    }
    else
    {
        conf.setString("feedback.show", "true");
        conf.setString("welcome.enable", "true");
        OXOOLWSD::MaxConnections = MAX_CONNECTIONS;
        OXOOLWSD::MaxDocuments = MAX_DOCUMENTS;
    }
#else
    {
        OXOOLWSD::MaxConnections = MAX_CONNECTIONS;
    ...
    if (supportKeyString.empty())
    {
        ...
        OXOOLWSD::OverrideWatermark = "Unsupported, the support key is missing.";
    }
    else
    {
        ...
        if (!key.verify())
        {
            ...
            OXOOLWSD::OverrideWatermark = "Unsupported, the support key is invalid.";
        }
        else
        {
            ...
            if (validDays <= 0)
            {
                ...
                OXOOLWSD::OverrideWatermark = "Unsupported, the support key has expired.";
            }
            else
            {
                LOG_INF("Your support key is valid for " << validDays << " days");
                OXOOLWSD::MaxConnections = 1000;
                OXOOLWSD::MaxDocuments = 200;
                OXOOLWSD::OverrideWatermark = "";
            }
        }
    }
#endif

    if (OXOOLWSD::MaxConnections < 3)
    {
        LOG_ERR("MAX_CONNECTIONS must be at least 3");
        OXOOLWSD::MaxConnections = 3;
    }

    if (OXOOLWSD::MaxDocuments > OXOOLWSD::MaxConnections)
    {
    ...
    LOG_INF("Maximum concurrent open Documents limit: " << OXOOLWSD::MaxDocuments);
    LOG_INF("Maximum concurrent client Connections limit: " << OXOOLWSD::MaxConnections);

    OXOOLWSD::NumConnections = 0;
    ...
    Server = std::make_unique<OXOOLWSDServer>();
    ...
    if (FILE* fp = fopen("/proc/self/smaps_rollup", "r"))
    {
        ...
        if (memoryDirty2 >= memoryDirty1 * 2)
        {
            ...
            setenv("OXOOL_DISABLE_SMAPS_ROLLUP", "1", true);
        }
        fclose(fp);
    }
...
void OXOOLWSD::dumpNewSessionTrace(const std::string& id, const std::string& sessionId, const std::string& uri, const std::string& path)
{
    if (TraceDumper)
    {
        try
...
void OXOOLWSD::dumpEndSessionTrace(const std::string& id, const std::string& sessionId, const std::string& uri)
{
    if (TraceDumper)
    {
        try
...
void OXOOLWSD::dumpEventTrace(const std::string& id, const std::string& sessionId, const std::string& data)
{
    if (TraceDumper)
    {
...
void OXOOLWSD::dumpIncomingTrace(const std::string& id, const std::string& sessionId, const std::string& data)
{
    if (TraceDumper)
    {
...
void OXOOLWSD::dumpOutgoingTrace(const std::string& id, const std::string& sessionId, const std::string& data)
{
    if (TraceDumper)
    {
...
void OXOOLWSD::defineOptions(OptionSet& optionSet)
{
#if !MOBILEAPP
    ...
    optionSet.addOption(Option("disable-oxool-user-checking", "", "Don't check whether oxoolwsd is running under the user 'oxool'.  NOTE: This is insecure, use only when you know what you are doing!")
                        .required(false)
                        .repeatable(false));
...
void OXOOLWSD::handleOption(const std::string& optionName,
                           const std::string& value)
{
    ...
    else if (optionName == "disable-oxool-user-checking")
        CheckOxoolUser = false;
    else if (optionName == "override")
    {
        std::string optName;
        std::string optValue;
        OXOOLProtocol::parseNameValuePair(value, optName, optValue);
    ...
    static const char* latencyMs = std::getenv("OXOOL_DELAY_SOCKET_MS");
    if (latencyMs)
...
void OXOOLWSD::initializeEnvOptions()
{
    int n = 0;
    char* aliasGroup;
...
bool OXOOLWSD::checkAndRestoreForKit()
{
...
void OXOOLWSD::closeDocument(const std::string& docKey, const std::string& message)
{
...
void OXOOLWSD::autoSave(const std::string& docKey)
{
    std::unique_lock<std::mutex> docBrokersLock(DocBrokersMutex);
    auto docBrokerIt = DocBrokers.find(docKey);
    if (docBrokerIt != DocBrokers.end())
...
void PrisonPoll::wakeupHook()
{
    ...
    if (!OXOOLWSD::checkAndRestoreForKit())
    {
...
//    args.push_back("--vgdb-error=0");
    ...
    if (!CheckOxoolUser)
        args.push_back("--disable-oxool-user-checking");
    ...
    const int balance = OXOOLWSD::NumPreSpawnedChildren - OutstandingForks;
    if (balance > 0)
                          ...
                          unsigned mobileAppDocId = 0)
{
    LOG_INF("Find or create DocBroker for docKey ["
            ...
            << OXOOLWSD::anonymizeUrl(uriPublic.toString()) << ']');
    ...
    {
        Util::assertIsLocked(DocBrokersMutex);
        if (DocBrokers.size() + 1 > OXOOLWSD::MaxDocuments)
        {
            LOG_WRN("Maximum number of open documents of "
                    << OXOOLWSD::MaxDocuments << " reached while loading new session [" << id
                    << "] for docKey [" << docKey << ']');
#if ENABLE_SUPPORT_KEY
...
public:
    ...
    ~PrisonerRequestDispatcher()
    {
        ...
        // replace it (once we receive 'GET /oxoolws/forkit') with
...
private:
    ...
    void handleIncomingMessage(SocketDisposition &disposition) override
    {
        ...
        {
#if !MOBILEAPP
            LOG_TRC("Child connection with URI [" << OXOOLWSD::anonymizeUrl(request.getUrl())
                                                  << ']');
            ...
            for (const auto& param : params)
            {
                if (param.first == "jailid")
                    ...
                    OXOOLWSD::LOKitVersion = param.second;
            }

            if (pid <= 0)
            {
                LOG_ERR("Invalid PID in child URI [" << OXOOLWSD::anonymizeUrl(request.getUrl())
                                                     << ']');
                return;
            }
            ...
            if (jailId.empty())
            {
                LOG_ERR("Invalid JailId in child URI [" << OXOOLWSD::anonymizeUrl(request.getUrl())
                                                        << ']');
                return;
            }
    ...
    virtual void handleMessage(const std::vector<char> &data) override
    {
        ...
        {
            const std::string abbreviatedMessage = OXOOLWSD::AnonymizeUserData ? "..." : message->abbr();
...
private:
    ...
    void handleIncomingMessage(SocketDisposition &disposition) override
    {
...
#if !MOBILEAPP
        if (!OXOOLWSD::isSSLEnabled() && socket->sniffSSL())
        {
        ...
        try
        {
            ...
            RequestDetails requestDetails(request, OXOOLWSD::ServiceRoot);
            // LOG_TRC("Request details " << requestDetails.toString());

            // Config & security ...
            if (requestDetails.isProxy())
            {
                if (!OXOOLWSD::IsProxyPrefixEnabled)
...
//              Util::dumpHex(std::cerr, socket->getInBuffer(), "clipboard:\n"); // lots of data ...
                ...
                handleClientWsUpgrade(request, requestDetails, disposition, socket);
                     ...
                     (requestDetails.equals(RequestDetails::Field::Type, "oxool") ||
                     requestDetails.equals(RequestDetails::Field::Type, "lool")))
            {
                // All post requests have url prefix 'oxool', except when the prefix
        ...
        catch (const BadRequestException& ex)
        {
            LOG_ERR('#' << socket->getFD() << " bad request: ["
                        << OXOOLProtocol::getAbbreviatedMessage(socket->getInBuffer())
                        << "]: " << ex.what());
        ...
        catch (const std::exception& exc)
        {
            LOG_ERR('#' << socket->getFD() << " Exception while processing incoming request: [" <<
                    OXOOLProtocol::getAbbreviatedMessage(socket->getInBuffer()) << "]: " << exc.what());
                              ...
                              const std::shared_ptr<StreamSocket>& socket)
    {
        ...
        if (!File(faviconPath).exists())
            faviconPath = OXOOLWSD::FileServerRoot + "/favicon.ico";
                                    ...
                                    const std::shared_ptr<StreamSocket>& socket)
    {
        ...
        std::string srvUrl =
#if ENABLE_SSL
            ((OXOOLWSD::isSSLEnabled() || OXOOLWSD::isSSLTermination()) ? "https://" : "http://")
#else
            "http://"
#endif
            + (OXOOLWSD::ServerName.empty() ? requestDetails.getHostUntrusted() : OXOOLWSD::ServerName)
            + OXOOLWSD::ServiceRoot;
        if (requestDetails.isProxy())
                           ...
                           const std::shared_ptr<StreamSocket>& socket)
    {
        ...
        LOG_INF("Post request: [" << OXOOLWSD::anonymizeUrl(requestDetails.getURI()) << ']');
        ...
        else if (requestDetails.equals(2, "insertfile"))
        {
            ...
            if (form.has("childid") && form.has("name"))
            {
                ...
                {
                    const std::string dirPath = OXOOLWSD::ChildRoot + formChildid
        ...
        else if (requestDetails.equals(2, "download"))
        {
            ...
            const Path filePath(OXOOLWSD::ChildRoot + jailId + JAILED_DOCUMENT_ROOT + decoded);
            const std::string filePathAnonym = OXOOLWSD::anonymizeUrl(filePath.toString());
                                  ...
                                  SocketDisposition &disposition)
    {
        ...
        LOG_INF("Starting Proxy request handler for session [" << _id << "] on url [" << OXOOLWSD::anonymizeUrl(url) << "].");
        ...
        LOG_INF("URL [" << OXOOLWSD::anonymizeUrl(url) << "] is " << (isReadOnly ? "readonly" : "writable") << '.');
                               ...
                               unsigned mobileAppDocId = 0)
    {
        ...
        try
        {
            if (OXOOLWSD::NumConnections >= OXOOLWSD::MaxConnections)
            {
                LOG_INF("Limit on maximum number of connections of " << OXOOLWSD::MaxConnections << " reached.");
#if ENABLE_SUPPORT_KEY
            ...
            LOG_INF("Starting GET request handler for session [" << _id << "] on url [" << OXOOLWSD::anonymizeUrl(url) << "].");
            ...
            LOG_INF("Sanitized URI [" << OXOOLWSD::anonymizeUrl(url) << "] to [" << OXOOLWSD::anonymizeUrl(uriPublic.toString()) <<
            ...
            LOG_INF("URL [" << OXOOLWSD::anonymizeUrl(url) << "] is "
    ...
    static std::string getDiscoveryXML()
    {
        ...
        if (!File(discoveryPath).exists())
        {
            ...
            discoveryPath = OXOOLWSD::FileServerRoot + "/discovery.xml";
        }
        ...
        const std::string uriBaseValue = rootUriValue + "/browser/" OXOOLWSD_VERSION_HASH "/";
        const std::string uriValue = uriBaseValue + "loleaflet.html?";
        ...
        for (unsigned long it = 0; it < listNodes->length(); ++it)
        {
            ...
            if (elem->getAttribute("name") == "edit")
            {
                ...
                if (OXOOLWSD::EditFileExtensions.insert(ext).second) // Skip duplicates.
                    LOG_DBG_S("Enabling editing of [" << ext << "] extension files");
            }
            else if (elem->getAttribute("name") == "view_comment")
            {
                const std::string ext = elem->getAttribute("ext");
                if (OXOOLWSD::ViewWithCommentsFileExtensions.insert(ext).second) // Skip duplicates.
                                    ...
                                    const std::shared_ptr<StreamSocket>& socket)
    {
        ...
        if (available)
            convert_to->set("endpoint", "/oxool/convert-to");
        ...
        capabilities->set("hasProxyPrefix", OXOOLWSD::IsProxyPrefixEnabled);
...
///
/// Waits for the connections from the oxools, and creates the
/// websockethandlers accordingly.
class OXOOLWSDServer
{
    OXOOLWSDServer(OXOOLWSDServer&& other) = delete;
    const OXOOLWSDServer& operator=(OXOOLWSDServer&& other) = delete;
...
public:
...
#if MOBILEAPP
        oxoolwsd_server_socket_fd = _serverSocket->getFD();
#endif
    ...
    void dumpState(std::ostream& os) const
    {
        ...
        os << "OXOOLWSDServer: " << version << " - " << hash
#if !MOBILEAPP
           << "\n  Kit version: " << OXOOLWSD::LOKitVersion
           << "\n  Ports: server " << ClientPortNumber << " prisoner " << MasterLocation
           << "\n  SSL: " << (OXOOLWSD::isSSLEnabled() ? "https" : "http")
           << "\n  SSL-Termination: " << (OXOOLWSD::isSSLTermination() ? "yes" : "no")
           << "\n  Security " << (OXOOLWSD::NoCapsForKit ? "no" : "") << " chroot, "
           << (OXOOLWSD::NoSeccomp ? "no" : "") << " api lockdown"
           << "\n  Admin: " << (OXOOLWSD::AdminEnabled ? "enabled" : "disabled")
           << "\n  RouteToken: " << OXOOLWSD::RouteToken
#endif
           << "\n  TerminationFlag: " << SigUtil::getTerminationFlag()
           << "\n  isShuttingDown: " << SigUtil::getShutdownRequestFlag()
           << "\n  NewChildren: " << NewChildren.size()
           << "\n  OutstandingForks: " << OutstandingForks
           << "\n  NumPreSpawnedChildren: " << OXOOLWSD::NumPreSpawnedChildren
...
#endif
           << "\n  vs. MaxDocuments: " << OXOOLWSD::MaxDocuments
           << "\n  NumConnections: " << OXOOLWSD::NumConnections
           << "\n  vs. MaxConnections: " << OXOOLWSD::MaxConnections
           << "\n  SysTemplate: " << OXOOLWSD::SysTemplate
           << "\n  LoTemplate: " << OXOOLWSD::LoTemplate
           << "\n  ChildRoot: " << OXOOLWSD::ChildRoot
           << "\n  FileServerRoot: " << OXOOLWSD::FileServerRoot
           << "\n  ServiceRoot: " << OXOOLWSD::ServiceRoot
           << "\n  LOKitVersion: " << OXOOLWSD::LOKitVersion
           << "\n  HostIdentifier: " << Util::getProcessIdentifier()
           << "\n  ConfigFile: " << OXOOLWSD::ConfigFile
           << "\n  ConfigDir: " << OXOOLWSD::ConfigDir
           << "\n  LogLevel: " << OXOOLWSD::LogLevel
           << "\n  AnonymizeUserData: " << (OXOOLWSD::AnonymizeUserData ? "yes" : "no")
           << "\n  CheckOxoolUser: " << (OXOOLWSD::CheckOxoolUser ? "yes" : "no")
           << "\n  IsProxyPrefixEnabled: " << (OXOOLWSD::IsProxyPrefixEnabled ? "yes" : "no")
           << "\n  OverrideWatermark: " << OXOOLWSD::OverrideWatermark
           << "\n  UserInterface: " << OXOOLWSD::UserInterface
            ;
        ...
        OXOOLWSD::SavedClipboards->dumpState(os);
#endif
...
private:
    ...
    std::shared_ptr<ServerSocket> findPrisonerServerPort()
    {
        ...
        if(!socket->link(OXOOLWSD::SysTemplate + "/0" + MasterLocation))
        {
        ...
        OXOOLWSD::prisonerServerSocketFD = socket->getFD();
        LOG_INF("Listening to prisoner connections on #" << OXOOLWSD::prisonerServerSocketFD);
#endif
        return socket;
    }
...
#if !MOBILEAPP
void OXOOLWSD::processFetchUpdate()
{
    try
    {
        ...
        uriFetch.addQueryParameter("version", OXOOLWSD_VERSION);
        ...
        if (httpResponse->statusLine().statusCode() == http::StatusCode::OK)
        {
            ...
            std::lock_guard<std::mutex> lock(OXOOLWSD::FetchUpdateMutex);
            OXOOLWSD::LatestVersion = httpResponse->getBody();
        }
        else
...
#if ENABLE_DEBUG
std::string OXOOLWSD::getServerURL()
{
    return getServiceURI(OXOOLWSD_TEST_OXOOL_UI);
}
#endif
#endif
...
#  endif
    ...
    assert(Server && "The OXOOLWSDServer instance does not exist.");
...
// No need to "have at least one child" beforehand on mobile
    ...
    if (!Util::isKitInProcess())
    {
        ...
        {
            int retry = (OXOOLWSD::NoCapsForKit ? 150 : 50);
    ...
    const std::string postMessageURI =
        getServiceURI("/browser/dist/framed.doc.html?file_path=" DEBUG_ABSSRCDIR
                      "/" OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_WRITER);
    std::ostringstream oss;
    oss << "\nLaunch one of these in your browser:\n\n"
        << "    Writer:      " << getLaunchURI(OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_WRITER) << '\n'
        << "    Calc:        " << getLaunchURI(OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_CALC) << '\n'
        << "    Impress:     " << getLaunchURI(OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_IMPRESS) << '\n'
        << "    Draw:        " << getLaunchURI(OXOOLWSD_TEST_DOCUMENT_RELATIVE_PATH_DRAW) << '\n'
        << "    postMessage: " << postMessageURI << std::endl;

    const std::string adminURI = getServiceURI(OXOOLWSD_TEST_ADMIN_CONSOLE, true);
    if (!adminURI.empty())
...
#if CODE_COVERAGE || VALGRIND_OXOOLFORKIT
...
std::shared_ptr<TerminatingPoll> OXOOLWSD:: getWebServerPoll ()
{
    return WebServerPoll;
}

void OXOOLWSD::cleanup()
{
    try
    {
        Server.reset();
...
int OXOOLWSD::main(const std::vector<std::string>& /*args*/)
{
...
int OXOOLWSD::getClientPortNumber()
{
...
std::vector<std::shared_ptr<DocumentBroker>> OXOOLWSD::getBrokersTestOnly()
{
    std::lock_guard<std::mutex> docBrokersLock(DocBrokersMutex);
...
std::set<pid_t> OXOOLWSD::getKitPids()
{
    std::set<pid_t> pids;
    pid_t pid;
    {
...
{
    OXOOLWSD::alertAllUsersInternal(msg);
}
...
#if !MOBILEAPP
    if (!Util::isKitInProcess() && OXOOLWSD::ForKitProcId > 0)
    {
        LOG_INF("Sending SIGUSR2 to forkit " << OXOOLWSD::ForKitProcId);
        ::kill(OXOOLWSD::ForKitProcId, SIGUSR2);
    }
#endif
