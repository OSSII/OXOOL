...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    auto trace = std::getenv("OXOOL_LOGLEVEL");
    if (!trace)
    ...
    NSString *oxoolwsdVersionHashFile = [cacheDirectory stringByAppendingPathComponent:@"oxoolwsd_version_hash"];

    NSData *oldCoreVersionHash = [NSData dataWithContentsOfFile:coreVersionHashFile];
    NSData *oldOxoolwsdVersionHash = [NSData dataWithContentsOfFile:oxoolwsdVersionHashFile];

    NSData *coreVersionHash = [NSData dataWithBytes:CORE_VERSION_HASH length:strlen(CORE_VERSION_HASH)];
    NSData *oxoolwsdVersionHash = [NSData dataWithBytes:OXOOLWSD_VERSION_HASH length:strlen(OXOOLWSD_VERSION_HASH)];

    if (oldCoreVersionHash == nil
        || ![oldCoreVersionHash isEqualToData:coreVersionHash]
        || oldOxoolwsdVersionHash == nil
        || ![oldOxoolwsdVersionHash isEqualToData:oxoolwsdVersionHash]) {
        ...
        if (![[NSFileManager defaultManager] createFileAtPath:oxoolwsdVersionHashFile contents:oxoolwsdVersionHash attributes:nil])
            NSLog(@"Could not create %@", oxoolwsdVersionHashFile);
    }
...
// This method is called when you use the "Share > Open in OxOffice" functionality in the
